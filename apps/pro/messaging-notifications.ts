...
async function prepareAndroidChannel(): Promise<void> {
  ...
  await Notifications.setNotificationChannelAsync("messages", {
    name: "Messages",
    description: "Organization, event, team, lesson, and rental messages",
...
async function expoToken(
  ...
  if (!easProjectId) {
    throw new Error("Duna Pro's notification project is not configured.");
  }
  return (
...
function openMessagingResponse(
  ...
  if (
    ...
    deepLink.startsWith("duna-pro://messages")
  ) {
    onMessage?.();
