...
export const metadata = { title: "Court reservation operations" };

export default async function CourtBookingOperationsPage({
  params,
}: {
  ...
  try {
    const [dashboard, workspace, detail] = await Promise.all([
      ...
      caller.operator.activityDetail({
        activityType: "court-booking",
        activityId,
      }),
    ]);
    return (
