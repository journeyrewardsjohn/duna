...
export function messagingPrincipalMode(
  request: Request,
  fallback: MessagingPrincipalMode = "organization",
): MessagingPrincipalMode {
  const requested = new URL(request.url).searchParams.get("asPrincipal");
  if (requested === "user") return "user";
  if (requested === "organization") return "organization";
  return fallback;
}
