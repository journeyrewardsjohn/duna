...
const database = openDatabaseAsync("duna-player-messaging.db").then(
  async (nextDatabase) => {
...
export function createPlayerMessagingOutbox(personId: string) {
  return new MessagingOutbox(
    new SQLiteMessagingOutboxStorage(`user:${personId}`),
  );
}
