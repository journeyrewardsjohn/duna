...
function Avatar({ avatar, size = 44 }: { avatar: HomeV3Avatar; size?: number }) {
  const palette = [c.navy, c.sand, c.sky, c.blush] as const;
  const backgroundColor = palette[
    Math.abs(
      avatar.id.split("").reduce((sum, character) => sum + character.charCodeAt(0), 0),
    ) % palette.length
  ];
    ...
    >
      ...
      {item.status === "going" ? (
        <View style={styles.goingStatus}>
          <DunaIcon color={c.positive} name="check" size={12} strokeWidth={1.8} />
        </View>
      ) : item.status === "action" ? (
        ...
        {game.booked ? (
          <View style={styles.bookedStatus}>
            <View style={styles.bookedCheck}>
              <DunaIcon color={c.positive} name="check" size={10} strokeWidth={2} />
            </View>
...
function MatchTeamRow({ team }: { readonly team: HomeV3MatchTeam }) {
  return (
    <View style={styles.matchTeamRow}>
      ...
      <View style={[styles.matchWinner, !team.winner && styles.matchWinnerEmpty]}>
        {team.winner ? (
...
export function HomeV3Screen(props: HomeV3Props) {
  ...
  const visibleUpcoming =
    activeTab === "training"
      ...
      : props.upcoming;
  const showCrew = (activeTab === "all" || activeTab === "circles") && props.crew;
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          ...
          >
            <DunaIcon color={c.ink} name="search" size={21} strokeWidth={1.45} />
          </Pressable>
          <Pressable
          ...
          >
            ...
            {props.notificationCount > 0 ? <View style={styles.notificationDot} /> : null}
          </Pressable>
        </View>
      </View>
      ...
      {showCrew ? (
        <View style={styles.sectionInset}>
          ...
          >
            ...
            <DunaIcon color={c.ink} name="arrow-right" size={18} strokeWidth={1.4} />
          </Pressable>
        </View>
      ) : null}
        ...
        <View style={styles.sectionInset}>
          ...
          <View style={styles.cardList}>
            {visibleUpcoming.length > 0 ? (
              visibleUpcoming.slice(0, 3).map((item) => (
              ...
              >
                ...
                <Text style={styles.emptyBody}>Find a game, court, or training session nearby.</Text>
              </Pressable>
            )}
          </View>
        </View>
          ...
          <View style={styles.openGamesList}>
            {props.openGames.length > 0 ? (
              props.openGames.slice(0, 2).map((game) => (
                <OpenGameCard game={game} key={game.id} />
              ))
            ) : (
              <Pressable onPress={props.onOpenMoreGames} style={styles.openEmpty}>
                <Text style={styles.openEmptyTitle}>No open runs tonight yet.</Text>
                <Text style={styles.openEmptyBody}>See nearby games or start one for your circle.</Text>
              </Pressable>
            )}
          </View>
          <Pressable onPress={props.onOpenMoreGames} style={styles.moreOpenGames}>
      ...
      {showMatches ? (
        <View style={styles.sectionInset}>
          <SectionHeader action="See all" onAction={props.onOpenMatches} title="Recent matches" />
          <View style={styles.cardList}>
            {props.recentMatches.length > 0 ? (
              props.recentMatches.slice(0, 3).map((match) => (
              ...
              <Pressable onPress={props.onOpenMatches} style={styles.emptyCard}>
                ...
                <Text style={styles.emptyBody}>Completed results and rating changes will appear here.</Text>
              </Pressable>
            )}
          </View>
          <Pressable onPress={props.onOpenMatches} style={styles.ratingStrip}>
            ...
            <View style={styles.ratingValues}>
              ...
              <Text
                style={[
                  styles.ratingDelta,
                  props.ratingDelta.startsWith("-") && styles.ratingDeltaNegative,
                ]}
              >
                {props.ratingDelta}
              </Text>
...
const styles = StyleSheet.create({
  ...
  avatarText: { color: c.page, fontSize: 11, fontWeight: "600" },
  ...
  avatarOverflowText: { color: c.navy, fontSize: 11, fontWeight: "600" },
  crewMessage: {
    color: c.ink,
    flex: 1,
    fontSize: 15,
  ...
  upcomingTitle: { color: c.ink, fontSize: 15, fontWeight: "600", lineHeight: 20 },
  upcomingMeta: { color: c.textTertiary, fontSize: 13, lineHeight: 17, marginTop: 2 },
  recurrencePill: {
  ...
  recurrenceText: { color: c.navyLift, fontSize: 10, fontWeight: "600", letterSpacing: 0.45 },
  goingStatus: {
    alignItems: "center",
  ...
  emptyBody: { color: c.textTertiary, fontSize: 13, lineHeight: 18, marginTop: 4 },
  openSection: {
  ...
  openSectionTitle: { color: c.page, fontSize: 20, fontWeight: "600", letterSpacing: -0.2 },
  openSectionMeta: { color: c.mist, fontSize: 13, lineHeight: 17, marginTop: 5 },
  openSectionMap: { color: c.mist, fontSize: 14, fontWeight: "500" },
  openGamesList: { gap: mobileGrid[2], marginTop: mobileGrid[3] },
  openGameCard: { backgroundColor: c.page, borderRadius: dunaAppShape.cardRadius, overflow: "hidden" },
  openGameHeader: {
    alignItems: "center",
    flexDirection: "row",
    justifyContent: "space-between",
  ...
  openGameTime: { color: c.ink, fontSize: 16, fontWeight: "600", letterSpacing: -0.15 },
  ...
  locationPinCenter: { borderColor: c.navy, borderRadius: 3, borderWidth: 1.3, height: 6, width: 6 },
  ...
  rosterDivider: { alignSelf: "center", backgroundColor: c.hairline, height: 64, width: 1 },
  ...
  openPlayerName: { color: c.navyLift, fontSize: 12, fontWeight: "500", marginTop: 4, maxWidth: 62 },
  openPlayerRating: { color: c.textTertiary, fontSize: 11, lineHeight: 14, marginTop: 1 },
  openGameFooter: { borderTopColor: c.subtleStrong, borderTopWidth: 1, flexDirection: "row" },
  openGameDetails: { flex: 1, paddingHorizontal: mobileGrid[3], paddingVertical: 13 },
  ...
  openEmpty: { backgroundColor: c.page, borderRadius: dunaAppShape.cardRadius, padding: mobileGrid[4] },
  openEmptyTitle: { color: c.ink, fontSize: 15, fontWeight: "600" },
  openEmptyBody: { color: c.textTertiary, fontSize: 13, lineHeight: 18, marginTop: 4 },
  moreOpenGames: { alignItems: "center", paddingBottom: 7, paddingTop: 14 },
  moreOpenGamesText: { color: c.page, fontSize: 14, fontWeight: "500" },
  matchCard: { backgroundColor: c.card, borderColor: c.hairline, borderRadius: dunaAppShape.cardRadius, borderWidth: 1, padding: mobileGrid[3] },
  matchCardHeader: { alignItems: "center", flexDirection: "row", gap: mobileGrid[2], justifyContent: "space-between" },
  matchKicker: { color: c.textTertiary, flex: 1, fontSize: 10.5, fontWeight: "600", letterSpacing: 0.6 },
  matchDelta: { color: c.textTertiary, fontSize: 12, fontVariant: ["tabular-nums"], fontWeight: "500" },
  matchDeltaPositive: { color: c.positive },
  matchTeams: { gap: mobileGrid[2], marginTop: mobileGrid[2] + 2 },
  matchTeamRow: { alignItems: "center", flexDirection: "row", gap: mobileGrid[2] },
  matchTeamCopy: { flex: 1, minWidth: 0 },
  matchTeamName: { color: c.ink, fontSize: 13.5, fontWeight: "500" },
  matchTeamRating: { color: c.textTertiary, fontSize: 11, marginTop: 1 },
  matchWinner: { alignItems: "center", backgroundColor: c.gold, borderRadius: 9, height: 18, justifyContent: "center", width: 18 },
  matchWinnerEmpty: { backgroundColor: "transparent" },
  setScore: { color: c.textFaint, fontSize: 15, fontVariant: ["tabular-nums"], minWidth: 24, textAlign: "center" },
  ...
  ratingLabel: { color: c.textTertiary, fontSize: 10.5, fontWeight: "600", letterSpacing: 0.8 },
  sparklinePlaceholder: { flex: 1, height: 30 },
  ratingValues: { alignItems: "baseline", flexDirection: "row", gap: 7, marginLeft: "auto" },
  ratingValue: { color: c.navy, fontSize: 22, fontVariant: ["tabular-nums"], fontWeight: "600" },
  ratingDelta: { color: c.positive, fontSize: 12, fontVariant: ["tabular-nums"], fontWeight: "500" },
  ...
  insightEyebrow: { color: c.navyLift, fontSize: 10.5, fontWeight: "600", letterSpacing: 0.8 },
  insightText: { color: c.navy, fontSize: 14, fontWeight: "500", lineHeight: 18, marginTop: 3 },
});
