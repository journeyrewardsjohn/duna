...
def test_builds_a_typed_rally_without_inventing_outcomes() -> None:
    ...
    # Ninety-percent usable coverage is not enough to claim a complete rally.
    assert all(contact.payload["rallySequenceComplete"] is False for contact in contacts)


def test_marks_rules_ready_only_for_a_proven_complete_rally() -> None:
    events = build_events(command(), output(usable=10_000_000))
    contacts = [event for event in events if event.eventType == "ball-contact"]

    # The bounded rally has a high-confidence visible landing, full usable
    # coverage, no >2-second observation gap, and neither edge is clipped.
    assert contacts
    assert all(contact.payload["rallySequenceComplete"] is True for contact in contacts)


def test_never_marks_a_clip_edge_or_gap_closed_rally_complete() -> None:
    edge = output(usable=10_000_000)
    edge.observations[0] = RawObservation(
        eventType="ball-contact",
        timeUs=500_000,
        confidence=0.95,
        contactKind="serve",
    )
    events = build_events(command(), edge)
    assert all(
        event.payload["rallySequenceComplete"] is False
        for event in events
        if event.eventType == "ball-contact"
    )


def test_proves_a_second_rally_after_an_inter_rally_gap() -> None:
    second = output(usable=12_000_000)
    second.sampledDurationUs = 12_000_000
    second.observations.extend(
        [
            RawObservation(
                eventType="ball-contact",
                timeUs=7_000_000,
    ...
    events = build_events(command(), second)
    second_rally = [
        event
        for event in events
        if event.eventType == "ball-contact" and event.sessionTimeUs >= 7_000_000
    ]
    assert second_rally
    assert all(event.payload["rallySequenceComplete"] is True for event in second_rally)

    gap = output(usable=10_000_000)
    gap.observations[1] = RawObservation(
        eventType="ball-contact",
        timeUs=3_300_000,
        confidence=0.92,
        contactKind="reception",
    )
    gap.observations[2] = RawObservation(
        eventType="ball-landing",
        timeUs=3_900_000,
        confidence=0.91,
        xMeters=2.5,
    ...
    events = build_events(command(), gap)
    assert all(
        event.payload["rallySequenceComplete"] is False
        for event in events
        if event.eventType == "ball-contact"
