...
from duna_vision_worker.schemas import QualityGate
from duna_vision_worker.validation import signed_envelope
...
def test_v2_attestation_verifies_the_exact_signed_payload(tmp_path: Path) -> None:
    model_sha = "a" * 64
    private_key = Ed25519PrivateKey.generate()
    ...
    gate = QualityGate(
        ...
        decision="passed",
        productionEligible=True,
        benchmarkId="real-match-held-out-v2",
        modelBundleSha256=model_sha,
    ...
    envelope = signed_envelope(gate, private_path)
    attestation_path = tmp_path / "attestation-v2.json"
    attestation_path.write_text(json.dumps(envelope))

    verified = load_verified_quality_gate(attestation_path, public_path, model_sha)
    assert verified.benchmarkId == "real-match-held-out-v2"

    envelope["payloadBase64"] = base64.b64encode(b"{}").decode()
    attestation_path.write_text(json.dumps(envelope))
    with pytest.raises(ValueError, match="signature is invalid"):
        load_verified_quality_gate(attestation_path, public_path, model_sha)
