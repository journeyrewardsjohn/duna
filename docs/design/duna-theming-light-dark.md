...
Duna already uses light and dark **semantically**. Light ground means *read this*. Dark ground means *this is live*. That is the zoning rule, and it's the spine of the whole system.
...
Theme sets **the pair**. Zone picks **which member of the pair**. In every theme, the performance zone is the darker, cooler, more accent-dense member. What changes between themes is *how* that difference is expressed.
...
**The invariant, stated once:** *editorial is always warmer and lighter than performance, in every theme, on every platform.* If a design breaks that, it's wrong regardless of how it looks.
...
<html data-theme="light">          <!-- or "dark" -->
  <section data-zone="editorial">  <!-- or "performance" -->
```
...
`--edge-light` is a 1px inset top highlight on performance surfaces only. It's the thing that makes a dark scoreboard read as *lit* rather than *empty*.
...
In light mode, elevation reads as *lighter and slightly shadowed*. In dark mode, shadows are invisible — you cannot cast a darker shadow onto near-black.
...
Elevation goes **lighter** as it rises, in both themes. That's the consistent mental model. What changes is the *supplementary* cue: light mode adds a soft shadow, dark mode adds a border and an edge-light.
...
Note the mobile dusk wash in dark mode: `#1A1517` is a warm plum-tinted near-black. The pink survives as *hue*, not as *lightness*. The planning-vs-competition temperature contrast holds in both themes, which is exactly what the mobile guide's temperature rule requires.
...
2. **Reversed lockup exists but recoloring is forbidden** → use the official white/reversed version at 100%, no opacity reduction. Skip the monochrome-at-rest step; the reversed asset *is* the at-rest state.
...
```css
:root,
[data-theme="light"] [data-zone="editorial"] { /* light editorial values */ }
...
- [ ] Every flare-colored *word* uses `--flare-text`, not `--flare`
...
- [ ] Status bar contrast correct during a zone change *within* a screen
