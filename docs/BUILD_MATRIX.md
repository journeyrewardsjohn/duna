...
| M10 Forms and waivers        | HQ, Web, Player               | Club-scoped immutable library, Markdown/PDF/DOCX import proposals, exact-text hash, guardian binding, post-purchase web/native signing, secure remaining-signer link, receipt evidence                                                               | Core workflow implemented; legal review, production email/provider configuration, and physical native-link validation remain release gates                                                                 |
...
| M15 Ask Duna and Operator    | Player, Pro, HQ               | CI-generated feature knowledge, organization brand context, persistent agent drafts, canonical hashes, clarification, court/coach-aware single-session reschedule, player notifications, one-time confirmation, golden trace, audit                  | Core governed copilot connected; provider credentials and signed native/device validation remain environment-specific                                                                                      |
